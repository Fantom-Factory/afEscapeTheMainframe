...
class Main {
	...
	static Void main() {
		AppBuilder("afEscapeTheMainframe") {
			 it.jarFiles = ["swt.jar"]
//			 it.platforms = ["win32-x86_64"]
		}.build |bob| {
			
			// create a new afEscapeTheMainframe.pod that contains the desktop music
			
			(bob._distDir + `lib/fan/afEscapeTheMainframe.pod`).rename("afTmp.pod")
			zout := Zip.write((bob._distDir + `lib/fan/afEscapeTheMainframe.pod`).out)
			
			zin  := Zip.read((bob._distDir + `lib/fan/afTmp.pod`).in)
			File? entry
			while ((entry = zin.readNext()) != null) {
				data := entry.readAllBuf
				out := zout.writeNext(entry.uri)
				out.writeBuf(data)
				out.flush
			}
			zin.close
			...
			zout.finish
			zout.close
			(bob._distDir + `lib/fan/afTmp.pod`).delete
		}
	}
}
