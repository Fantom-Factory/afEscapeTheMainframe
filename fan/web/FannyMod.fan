...
const class FannyMod : WebMod {
	...
	private const Str	windowDesc	:= "A tense jump game to test your reflexes, rendered in stunning retro 3D vector graphics; written in Fantom, created by Alien-Factory"
	...
	override Void onGet() {
		if (req.headers["X-Forwarded-Host"] == "fanny.fantomfactory.org") {
			res.redirect(`http://escape.fantomfactory.org/` + req.modRel, 302)
			return
		}
	...
	Void onIndexPage(WebOutStream out, Uri reqAbsUri) {
		...
		out.body
			...
			out.div("class='bgScreen'")
				...
				out.div("class='twits'")
					out.a(`https://twitter.com/intent/tweet`, "class='twitter-share-button' data-text='Escape the Mainframe - a tense jump game rendered in stunning retro 3D vector graphics, written in #FantomLang' data-url='http://escape.fantomfactory.org/'").w("Tweet").aEnd
				out.divEnd
			...
			out.script.w("if (screen.width < 784)
			                document.getElementById('myViewport').setAttribute('content','width=784');")
			out.scriptEnd
